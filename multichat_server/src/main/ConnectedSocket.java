	...
	private void requestMapping(String requestJson) {
		...
		switch(requestDto.getResource()) {
			...
			case "sendMessage":
				...
				sendToAll(new ResponseDto<String>("receiveMessage", username + ": " + requestDto.getBody()), room.getUsers());
				break;
	...
	private void checkUsername(String username) {
		...
		for(ConnectedSocket connectedSocket : connectedSocketList) {
			if(connectedSocket.getUsername().equals(username)) {
				sendToMe(new ResponseDto<String>("usernameCheckIsDuplicate", "이미 사용 중인 이름입니다."));
				return;
			}
		}
